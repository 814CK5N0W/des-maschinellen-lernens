...
class PolicyIterationAgent:
    ...
    def __init__(self, env):
        ...
        self.actions_variants = np.array([0,1])
        # Задание стратегии (политики)
        # Карта 4х4 и 4 возможных действия 
        self.policy_probs = np.full((self.observation_dim, len(self.actions_variants)), 0.5)
        ...
        for iterations in range(self.maxNumberOfIterations):
            ...
            for state in range(self.observation_dim):
                ...
                for action, prob in enumerate(action_probabilities):
                    ...
                    # print(self.env.P)
                    # print(state)
                    for probability, next_state, reward, isTerminalState in self.env[state][action]:
    ...
    def policy_improvement(self):
        ...
        for state in range(self.observation_dim):
            for action in range(len(self.actions_variants)):
                for probability, next_state, reward, isTerminalState in self.env.env.P[state][action]:
